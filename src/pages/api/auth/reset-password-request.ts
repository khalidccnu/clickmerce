...
async function handleResetPasswordRequest(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (settings.data?.is_email_configured && existingUser?.data?.email) {
      await mailerSendFn({
        ...
        data: {
          ...
          content: `Your ${settings?.data?.identity?.name || Env.webTitle} verification code is: ${otp}`,
        },
