...
import { profileVerifySchema, TProfileVerifyDto } from '@modules/auth/lib/dtos';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (method) {
    ...
    case 'PATCH':
      return handleVerifyProfile(req, res);
    default:
...
async function handleVerifyProfile(req: NextApiRequest, res: NextApiResponse) {
  const { success, data, ...restProps } = await validate<TProfileVerifyDto>(profileVerifySchema, req.body);

  if (!success) return res.status(400).json({ success, data, ...restProps });

  const { phone, otp, hash } = data;
  ...
  const response: IBaseResponse = {
    success: true,
    statusCode: 201,
    message: `Profile has been verified successfully for ${existingUser.data?.phone}.`,
    data: null,
    meta: null,
  };
