...
interface IProps {
  ...
  isShowActiveFilter?: boolean;
  initialValues: IProductsFilter;
  onChange: (values: IProductsFilter) => void;
}

const ProductsFilterDrawer: React.FC<IProps> = ({
  isOpen,
  onChangeOpen,
  ...
  const categoryQuery = CategoriesHooks.useFindById({
    ...
    config: {
      queryKey: [],
      enabled: isOpen && !!formValues?.category_id,
    },
  });
  ...
  const categoriesQuery = CategoriesHooks.useFindInfinite({
    options: {
      limit: '20',
      search_term: categorySearchTerm,
      search_field: 'name',
    },
  });
  ...
  const dosageFormQuery = DosageFormsHooks.useFindById({
    ...
    config: {
      queryKey: [],
      enabled: isOpen && !!formValues?.dosage_form_id,
    },
  });
  ...
  const genericQuery = GenericsHooks.useFindById({
    ...
    config: {
      queryKey: [],
      enabled: isOpen && !!formValues?.generic_id,
    },
  });
  ...
  const supplierQuery = SuppliersHooks.useFindById({
    ...
    config: {
      queryKey: [],
      enabled: isOpen && !!formValues?.supplier_id,
    },
  });
