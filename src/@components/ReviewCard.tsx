...
const ReviewCard: React.FC<IProps> = ({ className, review }) => {
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 dark:text-gray-300 text-sm md:text-base">{review?.comment}</p>
      ...
      {!review?.image || (
        <div className="image_wrapper absolute bottom-0 right-0 bg-gray-100 dark:bg-[var(--color-dark-gray)] dark:text-white px-2 py-1 rounded-tl-xl rounded-br-xl">
          {previewImage && (
            <Image
